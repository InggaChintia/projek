...
require_once 'connection.php'; // Sesuaikan dengan nama file dan lokasi koneksi Anda
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['response'])) {
    ...
    // Prepared statement untuk menyimpan jawaban ke database
    $stmt = $conn->prepare("INSERT INTO $table_name (responden_${role}_id, survey_id, soal_id, jawaban) VALUES (?, ?, ?, ?)");

    foreach ($responses as $soal_id => $jawaban) {
        $soal_id = intval($soal_id);
        $jawaban = intval($jawaban);
        // Bind parameters dan eksekusi statement untuk setiap jawaban
    ...
    $stmt->close();
    $conn->close();
    // Redirect ke halaman terima kasih setelah pengiriman berhasil
